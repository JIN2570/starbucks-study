...
toTopEl.addEventListener('click', function () {
  gsap.to(window, 0.6,{
    scrollTo: 0 // 페이지의 0px 지점(최상단)으로 이동, Scroll To Plugin을 연결해야 사용 가능한 옵션
  });
});
...
// 두번째 인자값: 다양한 옵션을 객체 데이터로 전달(다른 옵션들 API 페이지 참고)
new Swiper('.notice .swiper', {
  direction: 'vertical', // 수직 슬라이드(기본값 horizontal)
...
new Swiper('.promotion .swiper', {
  ...
  loop: true,
  autoplay: {
    delay: 5000 // 5초마다 슬라이드 바뀜(기본값이 3000)
  },
...
// gsap.to(요소, 지속시간, 옵션: {})
// 옵션 참고: https://greensock.com/doce/v3/GSAP/gsap.to()
...
gsap.to('.floating2', 2, {
  dalay: 0.5, // 얼마나 늦게 애니메이션을 시작할 것인지 지연 시간을 설정
  y: 15, // 수직으로 얼마나 움직일지 설정(translateY(수치); 와 같음)
  repeat: -1, // 몇 번 반복하는지를 설정, -1은 무한 반복
  yoyo: true, // 한 번 재생된 애니메이션을 다시 뒤로 재생
  ease: Power1.easeInOut // 타이밍 함수 적용, 느리게-빠르게-느리게
...
gsap.to('.floating3', 2.5, {
  dalay: 1.5, // 얼마나 늦게 애니메이션을 시작할 것인지 지연 시간을 설정
